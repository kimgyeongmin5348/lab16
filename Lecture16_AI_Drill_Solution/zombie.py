...
class Zombie:
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition('좀비가 약한가?', self.is_week)

        SEQ_flee_if_zombie_week = Sequence('좀비가 약하면 도망', c2, a6)

        SEL_flee_or_chase = Selector('도망 또는 추적', SEQ_flee_if_zombie_week, a4)

        SEQ_chase_boy = Sequence('소년을 추적', c1, SEL_flee_or_chase)
        ...
        c3 = Condition('공이 근처에 있는가?',self.ball_near,3)

        a7 = Action('공으로 접근',self.move_to_ball)

        root = SEQ_nearby_ball = Sequence('공이 근처에 있으면 공으로',c3,a7)
